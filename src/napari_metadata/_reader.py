...
METADATA_KEYS = ("name", "visible", "contrast_limits", "colormap", "color")
        ...
        for node in nodes:
            ...
            if data is None or len(data) < 1:
                ...
                for a in node.metadata["axes"]:
                    ...
                    if a["type"] == "space":
                        axis = SpaceAxis(
                            name=name, unit=SpaceUnits.from_name(unit)
                        )
                    elif a["type"] == "time":
                        axis = TimeAxis(
                            name=name, unit=TimeUnits.from_name(unit)
                        )
                    if axis is not None:
                        axes.append(axis)
